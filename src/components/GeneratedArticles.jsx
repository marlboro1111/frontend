...
import { generateArticle, suggestTitles } from '../features/articles/articlesSlice'; // ✅ Correct imports


const GeneratedArticles = () => {
 const dispatch = useDispatch();
 const { generatedText, generatedUrl, status, error, suggestedTitles } = useSelector((state) => state.articles);
 ...
 const handleSuggestTitles = () => {
   if (generatedText) {
     const promptText = titlePrompt.trim() ? `${titlePrompt}: ${generatedText}` : generatedText;
 ...
 const handleRunPythonFunction = async () => {
   try {
     const element = document.querySelector('.p-4.border.rounded-md.bg-gray-50');
     if (!element) {
       throw new Error('Element with the class "p-4 border rounded-md bg-gray-50" not found.');
     }
     ...
     const response = await fetch("http://localhost:5000/run-python-function", {
       method: "POST",
       headers: {
 ...
 return (
   <section id="generated-articles" ref={pageRef} className="p-6 bg-white shadow-lg rounded-lg">
